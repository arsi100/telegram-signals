...
# --- Output ---
OUT = Path(f"analysis_results/final_strategy_v2_{INTERVAL_PRIMARY}m"); OUT.mkdir(parents=True, exist_ok=True)
...
def get_data(sym: str) -> pd.DataFrame:
    ...
    df_primary['is_bullish'] = df_primary['close'] > df_primary['open']
...
def select_signals(df: pd.DataFrame, sym: str) -> pd.Index:
    """Selects entry signals with coin-specific filters."""
    base = (
    ...
    # Add coin-specific filter for volatile pairs
    if sym in ["SOLUSDT", "CROUSDT"]:
        base &= (df["is_bullish"] == True)
    ...
    for idx, row in df.iterrows():
        ...
        if idx in sig_idx and len(positions) < MAX_CONCURRENT:
            if not any(ts > row['ts'] - pd.Timedelta(minutes=30) for ts in active_trade_timestamps):
                positions.append({
                    'entry_price': row['open'], 'entry_ts': row['ts'], 
                    'scaled': False, 'peak': row['high']
                })
...
def main():
    ...
    print("\n--- Final Hybrid Strategy v2 (Filtered) Backtest ---")
